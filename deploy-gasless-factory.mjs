import { createThirdwebClient, prepareContractCall, sendAndConfirmTransaction, defineChain } from "thirdweb";
import { privateKeyToAccount, smartWallet } from "thirdweb/wallets";
import { ethers } from "ethers";
import fs from "fs";

const TARGET_CHAIN = defineChain(8453); // Base Mainnet
const SECRET_KEY = process.env.THIRDWEB_SECRET_KEY || "Z5Bk-8ouqvIVbAm1BrXt4cBJuHf1GfNyuRxCltJUEkJTeL2FZ6uUUSdRf53-FNonQRuikuGV5JrZI1lo7URH8Q";
const SIGNER_KEY = process.env.SIGNER_KEY || "59e11e8663a63647e2609c6ca9548b78aff5c5a33bcdd4447baf36d0e02f6162";

// Singleton CREATE2 Factory universale attivo su Base Mainnet (ERC-2470 / Nick's Method)
const FACTORY_ADDRESS = "0x4e59b44847b379578588920ca78fbf26c0b4956c";

// Bytecode di DirectClearingGateway (Solidity 0.8.20)
const BYTECODE = "";

async function main() {
  console.log("==================================================");
  console.log("DEPLOY GATEWAY VIA THIRDWEB PAYMASTER (ZERO GAS)");
  console.log("==================================================");

  const client = createThirdwebClient({ secretKey: SECRET_KEY });
  const personalAccount = privateKeyToAccount({ client, privateKey: SIGNER_KEY });

  const wallet = smartWallet({
    chain: TARGET_CHAIN,
    sponsorGas: true,
  });

  const smartAccount = await wallet.connect({ client, personalAccount });
  console.log("Smart Account Deployer :", smartAccount.address);

  // Salt univoco basato sul timestamp
  const salt = ethers.id(`DIRECT_CLEARING_${Date.now()}`);
  
  // Calcolo deterministico dell'indirizzo che verrà generato da CREATE2
  const computedAddress = ethers.getCreate2Address(
    FACTORY_ADDRESS,
    salt,
    ethers.keccak256(BYTECODE)
  );

  console.log("Indirizzo Target Calcolato :", computedAddress);

  // Il factory ERC-2470 riceve: salt (bytes32) + init_code (bytes)
  const factoryCallData = ethers.concat([salt, BYTECODE]);

  console.log("\nInvio UserOperation di Deploy sponsorizzata da Thirdweb...");
  
  const { getContract } = await import("thirdweb");
  const factoryContract = getContract({
    client,
    chain: TARGET_CHAIN,
    address: FACTORY_ADDRESS
  });

  const deployTx = prepareContractCall({
    contract: factoryContract,
    method: "function deploy(bytes calldata initCode, bytes32 salt) returns (address)",
    params: [BYTECODE, salt]
  });

  const receipt = await sendAndConfirmTransaction({
    transaction: deployTx,
    account: smartAccount
  });

  console.log("✔ Deploy completato a costo zero!");
  console.log(" Tx Hash               :", receipt.transactionHash);
  console.log(" Gateway Contrattuale  :", computedAddress);
  console.log(" Explorer Link         :", `https://basescan.org/address/${computedAddress}`);

  fs.writeFileSync("./deployed-gateway.json", JSON.stringify({ address: computedAddress }, null, 2));
  console.log("Salvato in ./deployed-gateway.json");
}

main().catch(console.error);
